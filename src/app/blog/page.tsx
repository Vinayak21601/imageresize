...
export default function BlogListingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-slate-900 selection:text-white flex flex-col">
      ...
      <div className="bg-sky-cloud-hero border-b border-zinc-200/60 pb-16 pt-4">
        <Navbar />

        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4 pt-8">
          <nav className="flex items-center justify-center gap-2 text-xs font-semibold text-slate-500">
            <Link href="/" className="hover:text-slate-900 transition-colors">Home</Link>
            <ChevronRight className="w-3.5 h-3.5 text-[#1E50F2]" />
            <span className="text-[#1E50F2] font-bold">Blog &amp; Guides</span>
          </nav>

          <h1 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight leading-tight">
            Image Processing <em className="font-serif italic font-normal text-slate-900">Blog &amp; Guides</em>
          </h1>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-[#1E50F2]">
            <Sparkles className="w-4 h-4 text-[#1E50F2]" />
            <span>Featured Article</span>
          </div>

          <div className="bg-white border border-slate-200/80 rounded-3xl p-6 sm:p-10 shadow-sm hover:shadow-md transition-all">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              ...
              <div className="lg:col-span-7 space-y-4">
                ...
                <h2 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight leading-snug">
                  <Link href={`/blog/${featuredPost.slug}`} className="hover:text-[#1E50F2] transition-colors">
                    {featuredPost.title}
                  </Link>
                </h2>
                ...
                <div className="pt-2 flex items-center justify-between">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-slate-900 hover:bg-black text-white text-xs font-bold transition-all shadow-xs active:scale-95 cursor-pointer"
                  >
              ...
              <div className="lg:col-span-5 flex justify-center">
                <div className="w-full bg-slate-50 border border-slate-200/80 rounded-2xl p-4 shadow-inner">
                  <img
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between border-b border-slate-200 pb-4">
            <h2 className="text-xl font-extrabold text-slate-900 tracking-tight flex items-center gap-2">
              <BookOpen className="w-5 h-5 text-[#1E50F2]" />
              <span>All Tutorials &amp; Articles</span>
            </h2>
            <span className="text-xs font-medium text-slate-500">{BLOG_POSTS.length} Articles</span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {regularPosts.map((post) => (
              <div
                key={post.slug}
                className="bg-white border border-slate-200/80 rounded-3xl p-6 space-y-4 shadow-sm hover:shadow-md transition-all flex flex-col justify-between"
              >
                <div className="space-y-3">
                  ...
                  <h3 className="text-lg font-bold text-slate-900 leading-snug">
                    <Link href={`/blog/${post.slug}`} className="hover:text-[#1E50F2] transition-colors">
                      {post.title}
                    </Link>
                  </h3>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs">
                  ...
                  <Link
                    href={`/blog/${post.slug}`}
                    className="font-bold text-[#1E50F2] hover:text-blue-700 flex items-center gap-1"
                  >
